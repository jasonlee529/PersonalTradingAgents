...
# Kimi (Moonshot) thinking-mode models accept the tools array but reject
# the ``tool_choice`` parameter (``tool_choice 'specified' is incompatible
# with thinking enabled``). Same pattern as DeepSeek: suppress tool_choice
# but still bind the schema as a tool. Reasoning content also needs round-trip
# echo on subsequent turns.
...
_BY_ID: dict[str, ModelCapabilities] = {
    ...
    # Kimi (Moonshot) — thinking-enabled models reject tool_choice
    "kimi-k3": _KIMI_THINKING,
    "kimi-k2-5": _KIMI_THINKING,
    "kimi-k2": _KIMI_THINKING,
}
